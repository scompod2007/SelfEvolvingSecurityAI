...
Responsibilities
----------------
• Connect collectors/file_monitor.py output to the
...
class FileMonitorIntegration:
    """
    Integration layer connecting the file monitor collector to the
    ...
    def __init__(
        ...
        """
        Initialize the File Monitor Integration.
        """
        self.logger = logger
        self.dispatcher = (
        ...
        self.logger.info("FileMonitorIntegration initialized.")
    ...
    def _forward_downstream(
        ...
        try:
            ...
            self.logger.exception(
                "FileMonitorIntegration downstream handler failed.",
                extra={
                    "correlation_id": correlation_id,
                    "event_type": event_type,
                },
    ...
    def process_event(self, event: dict[str, Any]) -> dict[str, Any]:
        """
        Process a single file telemetry event through the dispatcher
        ...
        event : dict
            ...
            Raw file telemetry event, typically produced by
            collectors/file_monitor.py. Expected fields include
            file_path, file_name, process_name, and timestamp.
        ...
        try:
            ...
            # Default event_type to FILE_CREATE when absent or empty
            raw_event_type = event_copy.get("event_type")
            if not isinstance(raw_event_type, str) or not raw_event_type.strip():
                event_copy["event_type"] = "FILE_CREATE"

            response = self.dispatcher.filter_file(event_copy)
            ...
            if accepted:
                self.logger.info(
                    "FileMonitorIntegration accepted event.",
                    extra={
                        "correlation_id": correlation_id,
                        "event_type": event_type,
                        "filter_name": filter_name,
                ...
                self.logger.info(
                    "FileMonitorIntegration filtered event.",
                    extra={
                        "correlation_id": correlation_id,
                        "event_type": event_type,
                        "filter_name": filter_name,
            ...
            self.logger.exception(
                "FileMonitorIntegration failed to process event."
            )
            with self._lock:
                self._update_metrics_locked(
                    accepted=False, forwarded=False, error=True
    ...
    def process_batch(
        ...
        """
        Process a batch of file telemetry events independently,
